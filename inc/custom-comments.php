...
add_filter('comment_form_default_fields', 'karyawp_bootstrap_comment_form_fields');

if (!function_exists('karyawp_bootstrap_comment_form_fields')) {
	/**
	 ...
	 * @return array
	 */
	function karyawp_bootstrap_comment_form_fields($fields)
	{
...
} // End of if function_exists( 'karyawp_bootstrap_comment_form_fields' )

// Add Bootstrap classes to comment form submit button and comment field.
add_filter('comment_form_defaults', 'karyawp_bootstrap_comment_form');

if (!function_exists('karyawp_bootstrap_comment_form')) {
	/**
	 ...
	 * @return string[]
	 */
	function karyawp_bootstrap_comment_form($args)
	{
		$replace = array(
			'<p class="' => '<div class="form-group mb-3 ',
...
} // End of if function_exists( 'karyawp_bootstrap_comment_form' ).


// Add note if comments are closed.
add_action('comment_form_comments_closed', 'karyawp_comment_form_comments_closed');

if (!function_exists('karyawp_comment_form_comments_closed')) {
	/**
	 * Displays a note that comments are closed if comments are closed and there are comments.
	 */
	function karyawp_comment_form_comments_closed()
	{
		if (get_comments_number() && post_type_supports(get_post_type(), 'comments')) {
?>
			<p class="no-comments"><?php esc_html_e('Comments are closed.', 'karyawp'); ?></p>
<?php
		}
	}
} // End of if function_exists( 'karyawp_comment_form_comments_closed' ).
