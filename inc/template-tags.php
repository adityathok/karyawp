...
if ( ! function_exists( 'zahro_entry_meta' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function zahro_entry_meta() {
		...
		$post_date = apply_filters(
			'zahro_entry_meta_date',
			sprintf(
				'<span class="post-meta-date">%1$s</span>',
				apply_filters( 'zahro_posted_on_time', $time_string )
			)
		);
		$post_author = apply_filters(
			'zahro_entry_meta_author',
			sprintf(
		...
		$post_meta_separator = apply_filters(
			'zahro_entry_meta_separator',
			sprintf(
				'<span class="post-meta-separator px-2">·</span>',
			)
		);

		echo '<span class="zahro_entry_meta">'.implode($post_meta_separator,$post_meta).'</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
